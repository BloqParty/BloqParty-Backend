...
util.timestampStringToNum = function (ts) {
    ...
    let conversions = [
        1000, 60000, 3600000, 86400000, 2592000000, 31536000000, 157700000000,
        1577000000000, 3154000000000, 15770000000000, 31540000000000,
    ];
    for (i in tsArr) {
...
util.convertMS = function (ms) {
    ...
    if (ms >= 2.3652e14) {
        ...
        while (ms >= 31540000000000) {
            obj.year = obj.year + 1000;
            ms = ms - 31540000000000;
        }
        while (ms >= 15770000000000) {
            obj.year = obj.year + 500;
            ms = ms - 15770000000000;
        }
        while (ms >= 3154000000000) {
            obj.year = obj.year + 100;
            ms = ms - 3154000000000;
        }
        while (ms >= 1577000000000) {
            obj.year = obj.year + 50;
            ms = ms - 1577000000000;
        }
        while (ms >= 157700000000) {
            obj.year = obj.year + 5;
            ms = ms - 157700000000;
        }
...
let convertMS = function (ms) {
    ...
    if (ms >= 2.3652e14 || ms === Infinity) {
        obj.infinite = 1;
    } else {
        obj.year = Math.floor(ms / 31536000000);
        ms = ms - obj.year * 31536000000;
        obj.month = Math.floor(ms / 2592000000);
        ms = ms - obj.month * 2592000000;
        obj.day = Math.floor(ms / 86400000);
        ms = ms - obj.day * 86400000;
        obj.hour = Math.floor(ms / 3600000);
        ms = ms - obj.hour * 3600000;
        obj.minute = Math.floor(ms / 60000);
        ms = ms - obj.minute * 60000;
        obj.seconds = Math.floor(ms / 1000);
        ms = ms - obj.seconds * 1000;
    }
    return obj;
};
...
let timeConvert = function (obj, setting, givenLimit) {
    ...
    if (obj.infinite) {
        ...
        if (limit === undefined) {
            limit = Object.entries(obj).length - 1;
        }
        let p = {
            year: " years",
            month: " months",
...
util.time = (content, shorten, { allowZero = false } = {}) => {
    let returnObject = {
        timestamp: `--:--`,
        string: ``,
        units: {
    ...
    if (typeof content == `string`) {
        if (content.includes(`:`)) {
            ...
            returnObject.timestamp = util.timestampConvert(
                returnObject.units,
                allowZero
            );
            returnObject.string = timeConvert(
                returnObject.units,
                shorten ? true : false,
                3
            );
        ...
        try {
            if (
                JSON.stringify(Object.keys(content).sort()) ===
                JSON.stringify(Object.keys(returnObject.units).sort())
            ) {
